...
public class PannelloEmozioni extends javax.swing.JPanel {
     ...
     */
    public PannelloEmozioni(String userId, String titolo, String autore, EmotionalSongs emo) {
        initComponents();
        this.userId = userId;
        this.titolo = titolo;
        this.autore = autore;
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (!isNumeric(amazementField.getText()) || !isNumeric(solemnityField.getText()) || !isNumeric(tendernessField.getText()) || !isNumeric(nostalgiaField.getText()) || !isNumeric(calmnessField.getText()) || !isNumeric(powerField.getText()) || !isNumeric(joyField.getText()) || !isNumeric(tensionField.getText()) || !isNumeric(sadnessField.getText())) {
            ...
            if ((amazement < 0 || amazement > 5) || (solemnity < 0 || solemnity > 5) || (tenderness < 0 || tenderness > 5) || (nostalgia < 0 || nostalgia > 5) || (calmness < 0 || calmness > 5) || (power < 0 || power > 5) || (joy < 0 || joy > 5) || (tension < 0 || tension > 5) || (sadness < 0 || sadness > 5)) {
                ...
                Emozione_Collection emozioni_lst = new Emozione_Collection(); // Creazione dell'oggetto Emozione_Collection, che conterrà le nuove emozioni 

                emozioni_lst.setIdutente(userId); // Setto chi ha messo le emozioni, quindi l'id utente
                ...
                // Inserisco i voti delle emozioni e note delle emozioni nell'oggetto
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("amazement", amazement, noteAmazementVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("solemnity", solemnity, noteSolemnityVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("tenderness", tenderness, noteTendernessVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("nostalgia", nostalgia, noteNostalgiaVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("calmness", calmness, noteCalmnessVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("power", power, notePowerVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("joy", joy, noteJoyVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("tension", tension, noteTensionVar));
                emozioni_lst.addEmozioniDatiToArrayList(new Emozioni_Dati("sadness", sadness, noteSadnessVar));

                // -----------------------------------------------
                try {
                    // TUTTO OK, chiamo il metodo per inserire le emozioni nell'XML
                    inserisciEmozioniBrano(emozioni_lst);
                } catch (TransformerException ex) {
                    JOptionPane.showMessageDialog(null, "Errore #F0001.ErroreMetodoOperaFile", "Errore", JOptionPane.ERROR_MESSAGE);
     ...
     */
    private void inserisciEmozioniBrano(Emozione_Collection emozioni_lst) throws TransformerException, TransformerConfigurationException, ParserConfigurationException, SAXException, IOException {

        OperazioniXML operazioniXML = new OperazioniXML(); // Creazione oggetto di tipo OperazioniXML
        Object check; // Oggetto di tipo Object che conterrà o l'errore in caso qualcosa sia andato storto durante la scrittura/lettura, altrimenti conterrà una booleana, vorrà dire che tutto è andato a buon fine
        check = operazioniXML.aggiornaXML(autore, titolo, emozioni_lst); // Chiamata al metodo aggiornaXML, metodo vero e proprio per lettura e riscrittura dell'XML aggiornato
        if (check instanceof Boolean) {
            ...
            JOptionPane.showMessageDialog(null, (String) check, "Errore", JOptionPane.ERROR_MESSAGE); // Output errore
