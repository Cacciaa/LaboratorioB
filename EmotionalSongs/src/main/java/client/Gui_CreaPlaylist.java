 ...
 * Creazione della playlist con le canzoni presenti nel databse mostrate
 ...
 * poi una volta terminata l'aggiunta dei brani si puo' confermare il salvataggio della playlist
 *
 */
public class Gui_CreaPlaylist extends javax.swing.JPanel {
    ...
    private ArrayList<Canzoni> listaCanzone, playlist;

    /**
     * Metodo costruttore che crea la nuova interfaccia Gui_CreaPlaylist, inizializzando le variabili
     * e riempendo la lista con le canzoni per la realizzazione di una playlist
     *
     * @param userId contentente il codice fiscale dell'utente
     * @param nomePlaylist contentente il nome della playlist da creare
     ...
     *
     */
    public Gui_CreaPlaylist(String userId, String nomePlaylist, EmotionalSongs emo, InterfacciaServizio is) {
        this.userId = userId;
        this.nomePlaylist = nomePlaylist;
        this.previousframe = emo;
        this.is=is;
        anteprimaPlaylist = "";
        listaCanzone = new ArrayList<Canzoni>();
        playlist = new ArrayList<Canzoni>();
        initComponents();
        
        
        DefaultListModel modelloLista = new DefaultListModel<>();
        jLstBrani.setModel(modelloLista);
        //aggiungo alla lista le canzoni messe a disposizioni dal server
        try {
            listaCanzone = is.getCanzoniForPlaylist();
        } catch (CanzoneInesistente ex) {
            JOptionPane.showMessageDialog(null, "Nessuna canzone disponibile", "Errore", JOptionPane.ERROR_MESSAGE);
        } catch (MyServerException | RemoteException ex) {
            Logger.getLogger(Gui_CreaPlaylist.class.getName()).log(Level.SEVERE, null, ex);
        }
        //aggiungo ogni canzone letta alla lista
        for (Canzoni c : listaCanzone) {
    ...
    private void initComponents() {
        ...
        jBtnSalvaSuFile.setText("<html> <h3> Salva");
    ...
    /**
     * Metodo che aggiunge i dati selezionati dalla lista cazioni alle caselle di testo, mostrandone i dettagli
     * Invocato al click di una canzone nella lista mostrata 
     *
        ...
        if (!evt.getValueIsAdjusting()) {
            ...
            for (Canzoni c : listaCanzone) {
                if (c.getTitolo().equals(selected)) {
                    ...
                    jTxtAnno.setText(String.valueOf(c.getAnno()));
                }
            }
    ...
    /**
     * Metodo che aggiunge i dati inseriti della nuova canzone all'oggetto playlist
     *
     * Invocato al click del bottone Aggiungi Canzone. Verifica che i TextField
     * siano riempiti e crea un oggetto canzone con ciò che è stato inserito,
     ...
     *
     * Invocato al click del bottone salva su file. Richiama il createPlaylist
     * passandogli nomePlaylist, playlist e lo userId se l'inserimento avviene con
     * successo mostra un messaggio di conferma altrimenti mostra un messaggio di errore nel TextArea
     *
     */
    public void RegistraPlaylist() {
        ...
        if (this.jLstAnteprima.getModel().getSize() == 0) {
            ...
            try {
                is.createPlaylist(nomePlaylist, playlist, userId);
                ...
                JOptionPane.showMessageDialog(null, "Dati errati", "Errore", JOptionPane.WARNING_MESSAGE);
            } 
            cleanAll();
